...
exports.create = function(req,res) {
    ...
    empleado.save(function(err){
        ...
        response.exito = true,
        response.msg = "El elmpleado se guardó correctamente"
        res.json(response)
    })
}
...
exports.remove = function(req, res) {
    Empleado.findByIdAndRemove(req.params.id, function(err) {
        if(err){
            console.error(err),
            response.exito = false,
            response.msg = "Error eliminando datos de usuario"
            res.json(response)
            return;
        }
        response.exito = true,
        response.msg = "El empleado se ha eliminado"
        res.json(response)
    })
}
